    ...
    SubShader
    {
        ...
        Pass
        {
			...
			Varyings AfterTessVertProgram (Attributes v)
			{
                ...
                float2 trailUV = (o.posWS.xz - _TrailFactor.xy) / _TrailFactor.zw + 0.5;

                float trailMap = SAMPLE_TEXTURE2D_LOD(_Trail, sampler_Trail, trailUV, 0).r;
                float height = trailUV.x > 0 && trailUV.x < 1 && trailUV.y > 0 && trailUV.y < 1 ? trailMap : 0;
            ...
            half4 FragmentProgram(Varyings i) : SV_TARGET 
            {
                ...
                half3 normalWS = normalize(i.normalWS);    //单位化法线
                half3 ambient = UNITY_LIGHTMODEL_AMBIENT.rgb * albedo; // 环境光

                Light light = GetMainLight();

                half3 diffuse = saturate(dot(light.direction, normalWS)) * /*light.color **/ albedo; // 漫反射
